...
def build_document():
    ...
    add_heading(doc, "Рейтинговая система", 1)
    p = doc.add_paragraph("Итоговый рейтинг формируется из результатов работы в течение семестра и экзамена.")
    format_paragraph(p, size=10.5, after=5)
    grading = [
        ("Практические занятия", 15, "Выполнение заданий, корректный результат, вывод и объяснение решения"),
        ("7 домашних заданий", 30, "Индивидуальные работы по основным разделам курса"),
        ("4 ML-квиза", 8, "Короткие проверки понимания ключевых понятий"),
        ("5 мини-собеседований", 5, "Устные ответы по основным алгоритмам и ML-ситуациям"),
        ("Kaggle", 10, "Валидация, модели, эксперименты, feature engineering и итоговый отчёт"),
        ("Итоговый проект", 12, "Полный ML pipeline и краткая защита"),
        ("Экзамен", 20, "Алгоритм, прикладная ML-ситуация и анализ результатов"),
        ("Итого", 100, "Максимальный основной рейтинг"),
    ]
    table = add_table(doc, ["Вид работы", "Баллы", "Что оценивается"], grading, [2.05, 0.75, 4.2], font_size=9.4, center_cols=(1,))
    ...
    homework = [
        ("HW1", "Первый корректный ML-эксперимент", 3),
        ("HW2", "KNN from scratch и scaling", 4),
        ("HW3", "Linear Regression, Gradient Descent и регуляризация", 5),
        ("HW4", "Logistic Regression, метрики и threshold", 5),
        ("HW5", "Decision Tree, Cross-Validation и Random Forest", 5),
        ("HW6", "Feature Engineering, Pipeline и Leakage Challenge", 4),
        ("HW7", "Clustering и PCA", 4),
    ]
    add_table(doc, ["Работа", "Тема", "Баллы"], homework, [0.8, 5.35, 0.85], font_size=9.4, center_cols=(0, 2))
    ...
    for item in (
        ...
        "Можно получить до 5 бонусных баллов; итоговый рейтинг не может превышать 100 баллов.",
        "Рекомендуемый обязательный минимум за экзамен — 8 из 20 баллов.",
    ):
        add_bullet(doc, item)

    add_heading(doc, "Шкала итоговой оценки", 1)
    add_table(
        doc,
        ["Баллы", "Оценка"],
        [("85–100", "Отлично"), ("71–84", "Хорошо"), ("60–70", "Удовлетворительно"), ("0–59", "Неудовлетворительно")],
        [1.5, 5.5],
        font_size=9.8,
        center_cols=(0,),
    )
